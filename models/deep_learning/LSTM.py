...
class LSTMRegressor(DLRegressor):
    ...
    The code is adapted from https://github.com/Anonymous-teams/Challenge-TeamJ/tree/master/LSTM_model
    """
    ...
    def build_model(self, input_shape):
        ...
        if (len(input_shape)>1):
            self.hist_len = min(32,input_shape[0])
        n_features = input_shape[-1]

        input_layer = tf.keras.layers.Input(input_shape)

        embedding = tf.keras.layers.Dense(n_features*4, input_shape=input_shape)(input_layer)
        embedding = tf.keras.layers.LeakyReLU()(embedding)

        hist_enc = tf.keras.layers.LSTM(n_features*2)(embedding)
        lstm_output= tf.keras.layers.Concatenate(axis=1)([hist_enc,embedding[:,-1,:]])

        output = tf.keras.layers.Dense(int(n_features*2.5))(lstm_output)
        output = tf.keras.layers.LeakyReLU()(output)
        output_layer = tf.keras.layers.Dense(1, activation='linear')(output)
    ...
    def fit(self, x_train, y_train, x_val=None, y_val=None, monitor_val=False):
        ...
        if len(x_train.shape) == 3:
            x_train = x_train[:,:self.hist_len,:] # x_train.reshape(x_train.shape[0], x_train.shape[1] * x_train.shape[2])
            x_val = x_val[:,:self.hist_len,:] # x_val.reshape(x_val.shape[0], x_val.shape[1] * x_val.shape[2])
    ...
    def predict(self, x):
        ...
        if len(x.shape) == 3:
            x = x[:,:self.hist_len,:] # x.reshape(x.shape[0], x.shape[1] * x.shape[2])
